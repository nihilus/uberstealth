...
#include "../../DynamicDriverName/DynamicDriverName.h"
#include <ntddk.h>
...
VOID STEALTHDRIVER_DriverUnload(IN PDRIVER_OBJECT DriverObject)
{
	...
	IoDeleteSymbolicLink(&DynamicDosDeviceName);
...
NTSTATUS DriverEntry(IN OUT PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
	...
	if (!initDeviceNames(RegistryPath)) return status;

	// Create the device object.
	if(!NT_SUCCESS(status = IoCreateDevice(DriverObject, 0, &DynamicDeviceName,
	...
	if(!NT_SUCCESS(status = IoCreateSymbolicLink(&DynamicDosDeviceName, &DynamicDeviceName)))
	{
