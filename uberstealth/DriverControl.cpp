...
void DriverControl::controlDriver(const std::string& driverPath, const std::string& driverName, bool load) const
{
	...
	SC_HANDLE hService = CreateService(hSCManager, StringToUnicode(driverName), StringToUnicode(driverName), SERVICE_ALL_ACCESS, SERVICE_KERNEL_DRIVER, SERVICE_DEMAND_START, SERVICE_ERROR_NORMAL, StringToUnicode(driverPath), NULL, NULL, NULL, NULL, NULL);
	if (!hService)
	{
...
void DriverControl::startDriver(const ResourceItem& ri, const std::string& driverName)
{
	...
	if (driverName.empty()) throw std::runtime_error("Error while trying to start driver: cannot start driver with empty name!");
...
void DriverControl::stopDriver()
{
	...
	DeleteFile(StringToUnicode(driverPath_));
	running_ = false;
}
...
void DriverControl::setMode(unsigned int ioctlCode, void* param, size_t paramSize) const
{
	...
	HANDLE hDevice = CreateFile(StringToUnicode(device), GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
